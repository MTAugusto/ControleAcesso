...
CREATE TABLE IF NOT EXISTS `controleacesso`.`movimentacao_veiculos` (
  ...
  `iscortesia` TINYINT(1) NULL,
  PRIMARY KEY (`id`, `usuario`, `veiculo`),
  INDEX `fk_saidas_usuarios1_idx` (`usuario` ASC),
  INDEX `fk_movimentacao_veiculos_veiculos1_idx` (`veiculo` ASC),
  CONSTRAINT `fk_saidas_usuarios1`
    FOREIGN KEY (`usuario`)
    REFERENCES `controleacesso`.`usuarios` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_movimentacao_veiculos_veiculos1`
    FOREIGN KEY (`veiculo`)
    REFERENCES `controleacesso`.`veiculos` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
